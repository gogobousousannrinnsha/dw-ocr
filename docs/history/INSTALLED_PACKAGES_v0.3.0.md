# 同梱パッケージのメタデータ

最終PortableのMETADATAから取得しました。vendored packageも含みます。許諾範囲は同梱LICENSE・NOTICE全文を参照してください。

| Name | Version | License表示 |
|---|---|---|
| aiohappyeyeballs | 2.7.1 | PSF-2.0 |
| aiohttp | 3.14.3 | Apache-2.0 AND MIT |
| aiosignal | 1.4.0 | Apache 2.0 |
| aistudio_sdk | 0.3.9 | Apache-2.0 |
| annotated-types | 0.8.0 | MIT |
| anyio | 4.15.1 | MIT |
| attrs | 26.1.0 | MIT |
| autocommand | 2.2.2 | LGPLv3 |
| backports.tarfile | 1.2.0 | 付属LICENSE参照 |
| bce-python-sdk | 0.9.79 | Apache License 2.0 |
| certifi | 2026.7.22 | MPL-2.0 |
| cffi | 2.1.1 | MIT-0 |
| chardet | 7.6.0 | 0BSD |
| charset-normalizer | 3.5.1 | MIT |
| click | 8.5.0 | BSD-3-Clause |
| colorama | 0.4.6 | 付属LICENSE参照 |
| colorlog | 6.12.0 | MIT License |
| crc32c | 2.9 | LGPL-2.1-or-later |
| cryptography | 50.0.1 | Apache-2.0 OR BSD-3-Clause |
| docuworks-ctypes | 1.0.0 | MIT |
| docuworks-integrations | 0.7.0 | MIT |
| filelock | 3.32.5 | MIT |
| frozenlist | 1.8.0 | Apache-2.0 |
| fsspec | 2026.7.0 | BSD-3-Clause |
| future | 1.0.0 | MIT |
| h11 | 0.16.0 | MIT |
| hf-xet | 1.6.0 | Apache-2.0 |
| httpcore | 1.0.9 | BSD-3-Clause |
| httpx | 0.28.1 | BSD-3-Clause |
| huggingface_hub | 1.30.0 | Apache-2.0 |
| idna | 3.19 | BSD-3-Clause |
| imagesize | 2.0.1 | MIT |
| importlib_metadata | 8.7.1 | Apache-2.0 |
| iniconfig | 2.3.0 | MIT |
| jaraco.context | 6.1.0 | MIT |
| jaraco.functools | 4.4.0 | MIT |
| jaraco.text | 4.0.0 | 付属LICENSE参照 |
| modelscope | 1.39.1 | Apache-2.0 |
| modelscope-hub | 0.4.0 | Apache-2.0 |
| more-itertools | 10.8.0 | MIT |
| multidict | 6.7.1 | Apache License 2.0 |
| networkx | 3.6.1 | BSD-3-Clause |
| numpy | 2.3.5 | Copyright (c) 2005-2025, NumPy Developers. |
| nvidia-cublas-cu12 | 12.9.0.13 | LicenseRef-NVIDIA-Proprietary |
| nvidia-cuda-runtime-cu12 | 12.9.37 | NVIDIA Proprietary Software |
| nvidia-cudnn-cu12 | 9.9.0.52 | NVIDIA Proprietary Software |
| nvidia-cufft-cu12 | 11.4.0.6 | NVIDIA Proprietary Software |
| nvidia-curand-cu12 | 10.3.10.19 | NVIDIA Proprietary Software |
| nvidia-cusolver-cu12 | 11.7.4.40 | NVIDIA Proprietary Software |
| nvidia-cusparse-cu12 | 12.5.9.5 | NVIDIA Proprietary Software |
| nvidia-nvjitlink-cu12 | 12.9.86 | LicenseRef-NVIDIA-Proprietary |
| opencv-contrib-python | 4.10.0.84 | Apache 2.0 |
| opt-einsum | 3.3.0 | MIT |
| packaging | 26.0 | Apache-2.0 OR BSD-2-Clause |
| packaging | 26.3 | Apache-2.0 OR BSD-2-Clause |
| paddleocr | 3.7.0 | Apache License 2.0 |
| paddlepaddle-gpu | 3.2.2 | Apache Software License |
| paddlex | 3.7.2 | Apache-2.0 |
| pandas | 3.0.5 | BSD 3-Clause License |
| pillow | 12.3.0 | MIT-CMU |
| pip | 26.2.1 | MIT |
| platformdirs | 4.4.0 | MIT |
| pluggy | 1.6.0 | MIT |
| prettytable | 3.18.0 | BSD-3-Clause |
| propcache | 0.5.2 | Apache-2.0 |
| protobuf | 6.33.4 | 3-Clause BSD License |
| psutil | 7.2.2 | BSD-3-Clause |
| py-cpuinfo | 9.0.0 | MIT |
| pyclipper | 1.4.0 | MIT |
| pycparser | 3.0 | BSD-3-Clause |
| pycryptodome | 3.23.0 | BSD, Public Domain |
| pydantic | 2.13.5 | MIT |
| pydantic_core | 2.46.5 | MIT |
| Pygments | 2.21.0 | BSD-2-Clause |
| pypdfium2 | 5.13.0 | BSD-3-Clause, Apache-2.0, dependency licenses |
| pytest | 9.1.1 | MIT |
| python-bidi | 0.6.11 | 付属LICENSE参照 |
| python-dateutil | 2.9.0.post0 | Dual License |
| PyYAML | 6.0.2 | MIT |
| requests | 2.34.2 | Apache-2.0 |
| ruamel.yaml | 0.19.1 | MIT |
| safetensors | 0.7.0 | 付属LICENSE参照 |
| setuptools | 84.0.0 | MIT |
| shapely | 2.1.2 | BSD 3-Clause |
| six | 1.17.0 | MIT |
| tomli | 2.4.0 | MIT |
| tqdm | 4.70.0 | MPL-2.0 AND MIT |
| typing-inspection | 0.4.4 | MIT |
| typing_extensions | 4.16.0 | PSF-2.0 |
| tzdata | 2026.3 | Apache-2.0 |
| ujson | 6.0.0 | BSD-3-Clause AND TCL |
| urllib3 | 2.7.0 | MIT |
| wcwidth | 0.8.3 | MIT |
| wheel | 0.46.3 | MIT |
| wheel | 0.48.0 | MIT |
| yarl | 1.24.5 | Apache-2.0 |
| zipp | 3.23.0 | MIT |
